...
- JSX 안에서는 함수를 전달 받아서 실행할 수 있다.
- JSX 안에서 함수로 JSX 엘리먼트(React.createElement)를 전달할 수 있다.
...
- JSX 안에서 함수 자체를 html 태그 처럼 사용할 수 있다.
- .propTypes를 이용해서 props의 유효성(validation) 검사를 할 수 있다.
- prop-types package 를 이용하면 .isRequired를 사용할 수 있다.
- JSX 표현식은 반드시 하나의 엘리먼트로 구성되어야 해서 불필요한 태그로 감싸야할
  때가 있다. 이때에는 React Fragments를 사용하자 (<></>)
- (<React.Fragments></React.Fragments>)나 (<></>)로 감싸주면 된다.
